...
class dttController {
    static getGeneticElementsFromInterval(leftEndPosition, rightEndPosition, strand, objectType, covered = false) {
        // When covered is true means draw only the elements that are contained in the selected range
        if (covered) {
            if (strand != 'both')
                // Return the elements that are completely contained in the selected range also when strand is "reverse or forward"
                return _dttModel.Data.find({ $and: [{ leftEndPosition: { $gte: leftEndPosition } }, { rightEndPosition: { $lte: rightEndPosition } }, { strand: strand }, { objectType: { $in: objectType } }] });else
                // Return the elements that are completely contained en the selected range also when strand is "reverse and forward"
        ...
        //When covered is false means draw both cases, when elements are contained in the range and those that not.
                ...
                //When strand is "reverse or forward"
                if (strand != 'both')
                    ...
                    // This function add to the position of the element a character "+" wich means if the position
